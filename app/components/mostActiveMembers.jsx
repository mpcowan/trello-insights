...
export default function ({ actions, actionsByCreator, before, since }) {
  ...
  const topMemberData = actionsByCreator.slice(0, 4).map(([idMemberCreator, memberActions]) => ({
    ...
    grouped: _(memberActions).countBy('type').pick(topActions).toPairs().value(),
  }));
  ...
  return (
    <div>
      ...
      <div className="spread-out">
        {topActions.concat(['other']).map((v, i) => (
          <ActionKeyChip key={i} hex={colors[i]} name={friendlyActions[v] || v} />
        ))}
      </div>
      ...
      <div className="spread-out">
        {topMemberData.map((memberData) => {
          ...
          const data = _.map(memberData.grouped, ([key, val]) => ({
            name: key,
            value: val,
          }));
          data.push({
            name: 'Other',
            value: memberData.count - _.sumBy(data, 'value'),
          });
          ...
          return (
            <div className="member-stat" key={`member-stat-${memberCreator.id}`}>
              ...
              <h4>
                <strong>{memberCreator.fullName}</strong>
              </h4>
